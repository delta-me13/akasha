...
//! ## 分层
//!
//! 纯逻辑（`acquire` / `cli` / `status` / `items` / `location` / `session`）没有一行
//! Tauri 依赖（`AGENTS.md` §3.1），于是"下载一个 45 MB 的压缩包、判定变体、解析状态"
//! 全都能脱离 app 测试；[`ipc`] 是它在 app 侧的那一半（命令与状态）。
//!
//! ## 模块边界
//!
//! | 模块 | 管什么 |
...
//! | [`status`] | 把 `bw status --raw` 的 JSON 翻成本模块的类型 |
...
//! - 不写回上游：本模块只有读与登录/锁定这几条命令，没有 `create` / `edit`。
