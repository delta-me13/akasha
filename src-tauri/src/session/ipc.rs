//! 会话命令 —— **薄壳**：把 `pty` 的批次接到 IPC，把前端输入送回载体。
//!
//! 这里**没有任何业务逻辑**（`AGENTS.md` §0 架构原则 2）：会话的归属、回收、事件路由
//! 在 [`crate::session::model`]；字节的进出与合批在 [`crate::pty`]。本模块只做三件编组的事：
//!
...
///
/// 与 `crate::session::SessionId` 分开还有一层理由：core 刻意不许外部构造 id
...
impl Sessions {
    ...
    /// `session::model` 之外构造（那是它的设计意图），所以壳层只用它的 checked 投影，
    ...
    /// 调整窗口尺寸（"尽力"语义 —— 载体可以拒绝，见 `pty` 模块的能力位）。
    ///
    ...
    pub fn open_tunnel(
        ...
        // 见 `crate::tunnel::TunnelState`），而订阅者该知道的正是"它开始连了"。
