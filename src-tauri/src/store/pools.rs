...
//!
//! ⚠️ **例外两处的措辞**：[`known_hosts`] 是**缓存**（plan 0503）—— 它确实随库走
//! （可搬迁的数据目录），但它不是用户配置的东西，而是我们替他记住的主机密钥。
//! [`bw_items`] 是 `scope.md` §7 的**导入池**（plan 0903）—— 它的行由导入产生、
//! 随 `keys` 行一起消失，不是"用户可以一格一格填"的第五套池。
//! 放在同一个模块里是因为它们同样是"库里的行 + 同样的不变量写法"。
//!
//! ## 三条贯穿四套池的约定
//!
...
//!    [`crate::store::schema`]）：库自己拦下来的东西，绕过任何代码路径都拦得住。
...
/// 它不读库，所以放在 [`crate::store::unlock`] 里不违反 D4 的顺序要求。
