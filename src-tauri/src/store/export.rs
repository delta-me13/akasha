...
//! 它不需要还原就能直接被 [`crate::store::open`] 打开（拿导出口令），也可以先还原到别处再开。
...
//!
//! 写完还要**自己开一次**：加密件用真正的读者 [`crate::store::open`]，明文件用裸 sqlite +
...
use crate::store::{Passphrase, StoreError, open, schema};
...
///
/// 只写进**空**的位置：`dest` 已经有内容就是 [`StoreError::VaultExists`]，与 [`crate::store::create`]
...
/// "还原后的库不能与它同口令"这条检查。
///
/// ⚠️ 来源用 [`crate::store::open_unmigrated`] 打开，**不是** [`crate::store::open`]：后者会为了迁移而
...
pub fn restore(
    ...
    let (conn, _version) = crate::store::open_unmigrated(source, source_passphrase)?;
...
/// 加密件写完再开一次 —— 用**真正的读者**（[`crate::store::open`]：送密钥、校验版本、
...
/// 与 [`crate::store::open`] 收到错口令时的说法一致（SQLCipher 对这两者本来就给同一个
/// `SQLITE_NOTADB`）。
///
/// **不迁移**：明文件可能就是用户抽屉里那份 v1 时代的导出，我们不去改写它（同
/// [`crate::store::open_unmigrated`]）。返回版本号让调用方原样抄走。
fn open_plaintext(path: &Path) -> Result<(Connection, i64), StoreError> {
    if crate::store::file_len(path)? == 0 {
        return Err(StoreError::NoVault(path.to_path_buf()));
    }
    let conn = Connection::open(path)?;
    let found: i64 = conn
        .query_row("PRAGMA user_version", [], |row| row.get(0))
        .map_err(crate::store::as_database_error)?;
