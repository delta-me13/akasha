...
//!    [`crate::store::unlock`] 每个连接都开一次，并由 `tests/schema_contract.rs` 读回断言。
//!
...
//!   失败整体回滚；由 [`crate::store::open`] 自动做（理由见 [`crate::store::upgrade`]）。
//! - 每个**历史**版本的表清单都要留在 [`tables_of`] 里：迁移要先按库里写的版本确认形状
//!   （[`crate::store::upgrade`] 的第一步），少了那一档，一个真 v2 库会被当成"不认识的版本"拒绝。
...
/// 建当前格式（v3）的全部表。**只在空库上跑**（[`crate::store::create`] 的路径）。
...
fn tables_of(version: i64) -> Option<&'static [&'static str]> {
    match version {
        ...
        v if v == crate::store::FORMAT_VERSION => Some(TABLES.as_slice()),
        _ => None,
    }
}
