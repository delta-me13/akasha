...
use crate::store::pools::import::Reason;
use serde::Serialize;
use tauri::State;

use crate::store::ipc::vault::{ConnError, Vault, VaultError};
...
pub fn vault_hosts(vault: State<'_, Vault>) -> Result<Vec<HostEntry>, VaultError> {
    let rows = vault
        .with_conn(crate::store::pools::hosts::hosts)
...
pub fn import_ssh_config(
    ...
    let parsed = crate::store::sshconfig::parse(&text, &user).map_err(|problems| {
    ...
    let outcome = vault
        .with_conn(|conn| {
            crate::store::pools::import::import_hosts(conn, &parsed.targets, overwrite)
        })
    ...
    let host = |row: crate::store::pools::import::Row| {
        host_id(row.id)
...
pub fn vault_forwards(vault: State<'_, Vault>) -> Result<Vec<ForwardEntry>, VaultError> {
    let rows = vault
        .with_conn(crate::store::pools::forwards::forwards)
        .map_err(VaultError::from_conn)?;
    rows.into_iter()
        .map(|row| {
            Ok(ForwardEntry {
