...
use crate::store::{Connection, Passphrase, StoreError, VaultState, dump, vault_path};
...
pub async fn vault_unlock(
    ...
    let state = crate::store::vault_state(&path).map_err(VaultError::from_store)?;
...
fn unlock(path: &Path, plan: Plan, passphrase: PassphraseInput) -> Result<Unlocked, StoreError> {
    ...
    let conn = match plan {
        Plan::Open => crate::store::open(path, &mut passphrase)?,
        Plan::Create => crate::store::create(path, &mut passphrase)?,
    };
...
pub fn vault_status(
    ...
    let path = vault_file(&app)?;
    let file = crate::store::vault_state(&path).map_err(VaultError::from_store)?;
