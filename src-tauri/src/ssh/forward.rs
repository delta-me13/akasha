...
//!
//! [`SshConnection`] **自己持有** `Handle`，所以句柄不出这个 crate，也不需要 [`crate::ssh::SshTransport`]
...
use crate::ssh::error::{SshError, forward_failed, remote_listen_failed};
use crate::ssh::handshake::{self, Handler, SshConnect};
use crate::ssh::remote::Inbound;
use crate::ssh::sftp::SftpClient;
use crate::ssh::target::{SshTarget, host_and_port};

/// `direct-tcpip` 通道的客户端一侧 —— **就是 D9 说的那条流**。
///
/// 为什么是新类型而不是把 `russh::ChannelStream` 直接漏进签名：`russh` 是 0.x，而"我们对外
/// 承诺的"是 `AsyncRead + AsyncWrite`（RFC 4254 §7.2 的通道语义，稳定）。与 [`crate::ssh::HostKey`]
...
/// 本身，而不是"一个带 pty 的 shell 终端"（那是 [`crate::ssh::SshTransport`]）。
...
impl SshConnection {
    ...
    ///
    /// 与 [`crate::ssh::SshTransport::connect_via`] 同一形状、同一约束（**不得在 tokio
    ...
    pub async fn sftp(&self) -> Result<SftpClient, SshError> {
        self.sftp_with_timeout(crate::ssh::sftp::SFTP_REQUEST_TIMEOUT_SECS)
            .await
    }
    ...
    /// 默认值（[`crate::ssh::sftp::SFTP_REQUEST_TIMEOUT_SECS`] = 10 s）对真人够用，
    ...
    /// （[`crate::ssh::SshConfig::keepalive_interval`] × `keepalive_max`，默认约 90 秒）才会变真。
    ...
    ///
    /// 只有 [`crate::ssh::RemoteForward::open`] 用它 —— 那里也是**唯一**会往里面登记的地方。
