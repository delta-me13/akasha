...
//! app 侧的 `tunnel.rs`，目标地址的类型在 [`crate::ssh::ForwardTarget`]。
//!
//! ## 三步，顺序是刻意的
//!
...
//! ## 谁持有那条连接
//!
//! 与 [`crate::ssh::LocalListener::serve`] 同一条纪律：[`RemoteForward::open`] **拿走**
...
use crate::ssh::ending::{ForwardEnd, ForwardEnding};
use crate::ssh::error::SshError;
use crate::ssh::forward::SshConnection;
use crate::ssh::relay::{ForwardTarget, LIVENESS_POLL_INTERVAL};
use crate::ssh::target::host_and_port;
...
impl Inbound {
    ...
    /// 收到服务端发起的通道 —— [`Handler`](crate::ssh::handshake::Handler) 的回调调它。
    ///
...
impl RemoteForward {
    ...
    /// 停止：**发完信号即返回**（理由与 [`crate::ssh::LocalForward::shutdown`] 相同）。
