...
pub enum SshError {
    ...
    /// 这次连接尝试**被调用方中止**（plan 0606）。
    ///
    /// 只有 [`crate::ssh::SshConnection::connect_via_until`] 会产生它：调用方把"关掉这条隧道"的
    /// 信号送了进来，于是那次握手（连同它已经建起来的 socket）当场结束。
    ///
    /// ⚠️ **与 [`SshError::Connect`] 分开**是刻意的：那不是"连不上"，而是"不要连了" ——
    /// 把它混进失败里，日志与界面就会报告一次**根本没发生过**的连接失败。
    #[error("连接被中止")]
    Cancelled,

    /// 服务端的主机密钥没被接受。**这条永远不该被自动重试**（ADR-0003 D13）。
    ///
    /// 两种情形共用一个变体：钉住的指纹对不上（[`crate::ssh::PinnedHostKey`]），
    ...
    /// （逐字节比，见 [`crate::ssh::KnownHostsVerifier`]），所以只能拒绝。
...
/// `direct-tcpip` 那条路的统一说法（原语在 [`crate::ssh::SshConnection::direct_tcpip`]）。
///
