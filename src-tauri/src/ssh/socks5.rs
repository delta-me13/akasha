...
//! [`crate::ssh::ForwardFailure`]（上游 `ChannelOpenFailure` 的结构化结果）翻过来，
//! 不从错误字符串里猜。
//!
...
use crate::ssh::relay::ForwardTarget;
...
/// "目标服务没起来"变得无法区分（见 [`crate::ssh::ForwardFailure`]）。
pub(crate) const fn reply_for(failure: crate::ssh::ForwardFailure) -> Reply {
    use crate::ssh::ForwardFailure as F;
    match failure {
