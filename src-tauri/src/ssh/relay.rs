...
//!
//! 这是 [`crate::ssh::SshConnection::direct_tcpip`]（D9 的原语）的第三个消费者 —— 前两处是
...
//! [`crate::ssh::socks5`]）。监听、每条入站连接一条通道、停止即回收这一整套形状两者共用。
//!
//! ## 先绑定，后连接
//!
...
//! 而 app 的那一个 runtime 是它在启动时建的（`crate::ssh::Ssh`）。
...
use crate::ssh::error::{SshError, listen_failed};
...
pub enum Ingress {
    ...
    Fixed(ForwardTarget),
    /// 动态转发 `-D`：目标由客户端在 SOCKS5 握手里说（[`crate::ssh::socks5`]）。
    Socks5,
}
