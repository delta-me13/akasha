...
use akasha_lib::store::pools::hosts::{self, Auth, NewHost};
use akasha_lib::store::pools::import::{Reason, import_hosts};
use akasha_lib::store::sshconfig::{Target, parse};
...
fn rows(conn: &akasha_lib::store::Connection) -> Vec<hosts::Host> {
...
fn an_identity_file_links_to_a_pool_key_of_the_same_name() {
    use akasha_lib::store::pools::keys;
...
fn a_cycle_handed_straight_to_the_store_is_refused_and_nothing_is_written() {
    ...
    assert!(
        matches!(refused, Err(akasha_lib::store::StoreError::JumpChain)),
