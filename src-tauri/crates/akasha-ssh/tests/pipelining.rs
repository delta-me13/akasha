...
use akasha_ssh::testing::{ServerOptions, slow_link, start};
use akasha_ssh::transfer::{
...
/// 连上测试服务端并开一个 SFTP 会话。
async fn connect_sftp(server: &akasha_ssh::testing::Running) -> akasha_ssh::SftpClient {
    connect_sftp_via(server, server.addr.port()).await
}

/// 同上，但客户端连的是**另一个端口**（带时延链路的入口）。
async fn connect_sftp_via(
    server: &akasha_ssh::testing::Running,
    port: u16,
) -> akasha_ssh::SftpClient {
