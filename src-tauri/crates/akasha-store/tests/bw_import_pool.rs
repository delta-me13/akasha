...
use akasha_store::pools::bw_items::{Incoming, Skip, import_snapshot, item, items};
use akasha_store::pools::keys::{self, PrivateKey};
...
fn pem_of(conn: &akasha_store::Connection, name: &str) -> String {
...
fn forgetting_the_record_keeps_the_key() {
    ...
    akasha_store::pools::bw_items::forget(&conn, "c-1").unwrap();
