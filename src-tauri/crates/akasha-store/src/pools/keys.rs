...
//! 记录，而那个错误要到连接时才出现。[`crate::schema`] 的 `private_pem BLOB NOT NULL`
...
use crate::protected::{PageError, Protected};
...
///
/// 刻意不实现 `Clone` / `Debug` / `Display`（与 [`crate::Passphrase`] 同一条理由）：
...
/// 把 [`PageError`] 翻成密钥这一侧的说法（与口令那一侧的 [`crate::passphrase`] 对称）。
...
/// `cipher_memory_security`，见 [`crate::unlock`]）；② 从行缓冲拷出来的那个 `Vec<u8>`，
