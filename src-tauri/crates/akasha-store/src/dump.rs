...
//! ## 为什么它**结构上**不含机密
//!
//! 私钥不在 [`Key`] 里：要它得单独走 [`crate::pools::keys::private_key`]（出来即进受保护页）。
...
use crate::pools::forwards::{self, Direction, Forward};
use crate::pools::hosts::{self, Host};
use crate::pools::keys::{self, Key};
use crate::pools::serial::{self, Parity, Serial};

/// 库里有什么：格式版本 + 四套池的全部行（**都不含机密**）。
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct Dump {
    /// `PRAGMA user_version` —— 读的是库里的真值，不是 [`crate::FORMAT_VERSION`]
