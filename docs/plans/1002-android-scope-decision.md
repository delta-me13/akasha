...
背景事实（决定了取舍的方向）：Android 上普通 app **无法访问 `/dev/ptmx`**（SELinux 沙箱）——
Termux 能使用 PTY 是因其自带 userland bootstrap，不属于普通 app 的路径（`scope.md` §2）。
...
- [ ] 「## 验收命令」：决定写入 `scope.md` 的具体章节 + docs-check 通过
- [ ] 若结论是"降级为只做 ssh/sftp"，把它登记为非目标/降级形态（而不是默认省略该范围）
