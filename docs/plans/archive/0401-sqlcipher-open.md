...
把"**用口令打开一个 SQLCipher 库**"这一件事做对；并把 ADR-0002 §7 的 7 项**实测执行完毕** ——
...
**库打不开**，而错误信息不会指向"少调了一次 key"。

## 非目标

- 口令的应用层封装、解锁界面、自动锁定（plan 0402）
- 四类池的表结构与 CRUD（plan 0403）
...
⚠️ **`unsafe` 的处理比预想的更复杂**（展开时实测）：workspace 是 `unsafe_code = "forbid"`，
而 rustc 规定 **`forbid` 不能被 `#[allow]` 覆盖** —— 也就是说 `AGENTS.md` §3.4 承诺的
"单点 `#[allow(unsafe_code)]` + `// SAFETY:` 注释"在当前配置下**根本写不出来**。
想"只在本 crate 放宽"也不可行：cargo **不允许部分覆盖**继承来的 lint
...
4. `empty_passphrase_refused_before_touching_file` —— 应用层拒绝；
   同文件另一例 `empty_key_really_disables_encryption` 用**直接调用 ffi** 证明 D5 的理由不是猜测
...
- 另外记一条 D12 相关的事实：**SQLite 默认建出的库文件权限是什么**（不是 0600 就说明
  0600 必须由数据目录那一侧显式设置，plan 0405 不能假设它已经是）。

### 5. 门禁

`just ready` 全部通过；`cargo tree -p akasha-store` 里出现 `openssl-src`（vendored，不是系统
...
| D12 的权限 | SQLite 自己建出来是 **644**；`open()` 之后 **600**（确实生效） |

**与 ADR 预期不符的只有一处**：D5 把空 key 说成"关闭加密"，实测机制是
`sqlite3_key_v2` 在 `nKey == 0` 时**直接 `return SQLITE_ERROR`**、根本不挂 codec
（源码 `libsqlite3-sys-0.30.1/sqlcipher/sqlite3.c:107794`）。**风险反而更高**：
