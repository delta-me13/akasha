...
库内（`just test transfer_atomic`，6 条，全部通过）：成功那条用两个**可控的假端点**把
"传到哪一步"变成可以等待的事件 —— 临时文件建好时报一次信号、源交出第一块后停住。
实测：那一刻目标目录里**只有** `.payload.bin.part`、长度正好是 `CHUNK_BYTES`，
最终名不存在；放行之后最终名的字节与源逐字节相同、临时名消失。取消那条把源永久停住，
取消之后目标目录**一个条目都没有**；写失败那条在第 2 次写入报错，同样一个条目都没有。
