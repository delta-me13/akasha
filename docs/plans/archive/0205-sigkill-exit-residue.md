...
**看门狗就是 app 可执行文件本身再跑一次**（`--akasha-session-watchdog`），它读一条管道，
**读到 EOF 就把登记过的会话全部收掉**：

| 环节 | 位置 |
|---|---|
| 协议（`+<pid>` / `-<pid>`）+ 解析 + 主循环 + 起进程 | `akasha-pty/src/watchdog.rs` |
...
| 认领 argv、启动、与终端脱钩 | `src-tauri/src/watchdog.rs` + `main.rs` 第一行 |

关键性质：**触发信号是管道 EOF**（由进程的 fd 表决定），不是任何"父/子"关系 ——
所以 app 怎么死都无所谓（SIGKILL / SIGTERM / abort / 段错误），也不关心会话是从哪个线程起的。

`PDEATHSIG` 被否掉的两条硬伤（ADR-0005 §4）：① 只作用于**直接子进程**，带不走 shell 里
忽略 SIGHUP 的孙进程 —— 而那正是本 plan 的验收判据；② 它的"父"是**创建它的那个线程**，
从线程池里起会话就会"A worker 线程退下去 → 误杀用户的会话"。
...
2. **`akasha-pty::watchdog`**：协议编解码、`run`（读协议 → EOF → 逐个 `kill_session`）、
   `detach`（`setsid`，让 `^C` 与关终端带不走它）、`SessionWatchdog`（起进程、`watch`/`forget`）。
...
5. **启动顺序**：看门狗在最前面起（会话一存在就必须已经在兜底清单里），但**日志推迟到
   `.setup()`** —— 早于日志插件注册的 `tracing` 事件会**静默消失**（实测踩到过）。
6. **测试**：`akasha-pty` 的真进程用例（含诱饵）、`src-tauri` 的协议断言用例、
   一条**真 SIGKILL** 的集成用例（真 app 二进制 + 真管道 + 真会话）。

## 验收命令

```bash
cargo test --package akasha --test session_watchdog   # 真 SIGKILL + 真 app 二进制
...
（能跑代码的两条路径干净、SIGKILL/SIGTERM 路径有残留）。

## 未覆盖（照实记，不假装"零"）

- **没有门禁的那条路径**：`tauri dev` 的重编译重启只能靠**手动**实测（`just test-e2e` 起的是
  自己的 app，改不了它的源码去触发重载）。机制本身有门禁 —— `tests/session_watchdog.rs`
  用**真 SIGKILL** 打真会话，那条用例在 `just test` 里。**但"CLI 真的用 SIGKILL"这件事
...
| 位置 | 内容 |
|---|---|
| `akasha-pty/src/watchdog.rs`（新） | `FLAG` / `is_invocation` / `Verb` / `parse` / `encode` / `run`（读协议 → EOF → 逐个 `kill_session`）/ `detach`（`setsid`）/ `SessionWatchdog`（起进程、`watch`、`forget`、收尸线程） |
...
| `src-tauri/src/watchdog.rs`（新） | `run_if_watchdog`（认领 argv → `detach` → 主循环）、`start_early` / `Startup` / `report` |
| `src-tauri/src/main.rs` | **第一行**认领看门狗模式（早于 Tauri，否则已经起了一个窗口） |
| `src-tauri/justfile` | E2E 清单加 `E2E_NO_APP`：不需要真 app 的集成测试有**显式的家**，不再被 guard 判红 |
...
- `cargo test --package akasha --test session_watchdog` —— **1 passed**（0.15 s）：
  真 app 二进制 + 真管道 + **真 SIGKILL** + 忽略 SIGHUP 的探针 + **诱饵会话**
  （从没登记过的那条必须毫发无伤），结束后还断言看门狗自己收工
...
脚本临时写在 `target/`（跑完即弃：它要真起 `pnpm tauri dev`、真要改一个 Rust 文件）。
...
每条路径的 app 日志里都有"兜底真的在位"的证据（这是"不是碰巧过了"的区别）：
...
1. **早于日志插件的 `tracing` 事件会静默消失**：看门狗起得比 `tauri-plugin-log` 早，
   第一版把"已启动"当场打出去 —— app 日志里**一个字都没有**（只有 victauri 的 INFO）。
   于是拆成 `start_early`（起进程 + 挂上）与 `report`（`.setup()` 里再报）。
2. **SIGKILL 之后立刻读 `/proc/<pid>/stat` 会读到 `R` 或 `Z`**：`R` 是信号还没投递，
   `Z` 是父进程还没 `wait`。把 `Z` 当"还活着"让两条**已经成功**的路径报红了一次 ——
   与坑 #45 同族，只是换了对象。
3. **"命令行里含 `sleep 600`"会匹配到沙箱包装进程**：bwrap 自己的 cmdline 里带着整段
   脚本文本，看起来很唬人。判据必须收紧到"argv 恰好是 `sleep 600`"。
