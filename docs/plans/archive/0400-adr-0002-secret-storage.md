...
定值时真正做的选择（其余取自 `scope.md` §6 的已定案）：

| 问题 | 取值 | 关键理由 |
|---|---|---|
| 库文件个数 | 一个（四类池 + BW 缓存） | 一次解锁、一次导出、跨池事务 |
...
**本次裁定另外改了一条流程**（不是本文档的临时变通）：ADR 原先只有"接受 / 不接受"两态，
而"接受"同时意味着"可以开工"**与**"不可再修改" —— 实现期一旦发现架构问题就无路可走：
要么带着已知架构问题实现，要么违规改 ADR。裁定为三态：
...
**未决的实测项**（写在 ADR §7，由 plan 0401 在写第一行存储代码之前运行）：
