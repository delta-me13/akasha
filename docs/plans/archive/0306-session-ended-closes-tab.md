# Plan 0306: 会话自己结束 = 收掉它 + 关掉那个标签页

- **关联**：ROADMAP 阶段 3 ·「会话自己结束（敲 exit）= 收掉它 + 关掉标签页」
- **前置**：plan 0305（标签页与会话一一对应）、0204（显式 kill + wait 收尸）、0205（撤销兜底登记）
- **状态**：已完成
...
而且后端那张会话表里还留着它 —— 一个已经死掉的会话**谁也收不掉**（它只是"看起来还在"）。

1. **后端当场收尾**：载体的输出流结束（shell 退出 / PTY 关闭）时立刻
   `kill + wait 收尸`、摘牌、`SessionRegistry::close`、**撤销看门狗登记** ——
   与"用户关标签页"走的是同一套收尾，只是触发者不同。
2. **发一个事件**告诉前端"这个会话自己结束了"（`SessionEnded`，按 handle 寻址）。
3. **前端把那个标签页关掉**：标签页与会话**同生命期**（0305 立的一一对应），
   两个方向都要成立 —— 用户关标签页 → 丢弃会话；会话自己走 → 标签页跟着走。

## 非目标

- **不做"已退出"的中间态**（灰掉的标签页、`[进程已结束]` 之类的提示）—— 直接关掉。
...
```bash
grep -n "forward(\|spawn_batcher" src-tauri/src/session.rs        # 输出流结束 → forward 线程收工
...
2. **`Sessions::retire(handle)`**：摘牌 + `transport.shutdown()`（收尸）+ `registry.close`
   + `forget(leader)`。**必须幂等**（`Ok(None)` = 已经不在了）：用户点 × 与 shell 自己退出
   可能几乎同时发生。收尾失败只记日志，**不半途而废**（会话已经结束，留着登记更糟）。
3. **`open_session` 加一个收尾线程**：`forward(...)` 的句柄 `join` 之后再 `retire` + 发事件。
   **顺序要紧**：`forward` 结束 = 最后一批已经交给频道 —— 事件**排在那之后**，
   否则前端可能"先收到会话结束、后收到最后一段输出"。
4. **前端订阅**（`src/ipc/session.ts`）：一个全局监听 + `handle → 回调`表；
   `TerminalSession` 记住"已结束"，此后 `write` / `resize` / `close` 都是**空操作**
   （后端已经摘牌，再发命令只会收到 `NotFound`）。
   还要兜住一个窄窗口：事件可能**早于**订阅到达（会话开起来就立刻死）——
...
6. **E2E**（`src-tauri/tests/tab_close.rs` 续一段）：在干净标签页里敲 `exit` →
   标签页自己关掉（0 个 = 空状态）、app 仍在；再开一个还能用（给 `exit_residue` 留个能用的终端）。
...
**回滚之后行为退回 0305 之前**：会话自己结束时无人收尾（表里留一条死会话）。

## 实施记录

| 命令 | 结果 |
|---|---|
| `just ready` | **6/6 全绿**（含 `gen-types-check`：新事件已生成并提交） |
| `just test` | **70 tests run: 70 passed**（新增 3 条 `retire` 单测：收干净 / 幂等 / 收尸失败也摘牌） |
| `just test-e2e` | 退出码 **0**，10 个用例全绿（`tab_close` 多验了一段"敲 exit"） |
| `pnpm build`（tsc + vite build） | 退出码 0 |

真 app 上的实测（`tab_close` 的新段落 + app 日志里那一行）：
...
—— 不引 `derive` 特性，那要多一个 proc-macro 依赖）+ `Sessions::retire`（幂等、不半途而废）
...
- 事件与最后一批输出的**严格顺序**没有保证：事件排在 `forward` 收工之后发，但频道与事件是
  两条通道。极端情况下最后一屏可能来不及画就被关掉（敲 `exit` 本来就没什么可看）。
- 在**有后台作业**的标签页里敲 `exit` 不会关标签页（见坑 3）—— 这是刻意的，但没有用例守着
