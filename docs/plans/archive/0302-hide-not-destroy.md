...
1. **不挂 `RunEvent::ExitRequested`，也不写 `prevent_exit`**（原步骤 2 取消）。
   ...
   `CloseRequested` 里拦下（窗口**不会被销毁**），轮不到它兜。剩下唯一会触发
   `ExitRequested` 的窗口路径是"窗口被 `destroy()` 强拆"，而那时窗口已经没了、
   从托盘也叫不回来 —— 放行（退出 + 回收）才是对的（与坑 #60 同一条道理）。
...
2. 判据没留在 app 侧：`配置 × 托盘可用性 → 动作` 是个纯函数，落在
   ...
   —— 记住启动期事实、接窗口事件、给 Victauri probe 一份快照。

**实测**（Linux/CachyOS + niri + waybar；沙箱里 `$XDG_RUNTIME_DIR` 只读，用
`XDG_RUNTIME_DIR=/tmp/akasha-run` + `wayland-1` 软链绕过；app 与测试在**同一次 bash 调用**里，坑 #33）：
...
| 隐藏期间 | 屏幕内容照旧读得到（`screenText` 仍含关窗前的标记）；忽略 SIGHUP 的探针**仍活着**（**预期行为，不是泄漏**） |
