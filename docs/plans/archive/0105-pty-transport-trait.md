...
其实什么也没证明 —— 回显里就有。探针因此写成 `printf 'AKASHA%s\n' _PROBE`：
回显里只有 `AKASHA%s` 与 `_PROBE` 两截，**只有 shell 真的执行了**才会拼出完整的那串。

读取按**截止时间 + 读端驱动**，不用固定 `sleep`（`AGENTS.md` §0 第 5 条）。
`shutdown` 之后断言三件事：结局非空（说明 `wait` 收尸过了）、重复 `shutdown` 幂等、
再 `write` 返回 `Closed`。

### 刻意与初稿不同的两处

- **能力位里没有 `signals`**。初稿的例子里有它，但 trait 现在**没有任何方法能发信号** ——
  没有对应方法的能力位是空头支票。等有了 `signal()` 再加（`resize` / `exited` 的默认实现
  就是现成模板）。
- **不实现 `Drop`**。`AGENTS.md` §3.3 说 drop 不能代替 kill + wait；在 `Drop` 里悄悄 kill
  会让"调用方忘了 `shutdown`"变成静默成功 —— 而那是要**暴露**的事，不是要兜住的事。
  代价写进了类型文档：不调 `shutdown()` 就会留下子进程。

### 走查中发现并修掉的一个真问题

`#[cfg(unix)] mod tests` 少了 `test` ⇒ **测试代码被编进正式产物**（只在 unix 上），
并因"没人用"报 `dead_code` 警告。已改为 `#[cfg(all(test, unix))]`。
这类"缺一个 cfg 就把测试带进产物"的问题，编译能过、测试也能过，只有看警告才发现。

### 两件**不做**的事（连同理由）

- **不把 `akasha-pty` 加进 `src-tauri` 的依赖**：现在没有任何代码用它，加了就是一条没人用的依赖。
  阶段 2 的 plan 0201 / 0202 接上时再加 —— 那时也应顺手删掉 `src-tauri` 里 `portable-pty` 的
