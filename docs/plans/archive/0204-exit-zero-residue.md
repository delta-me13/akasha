...
**真正退出**时每个 PTY 子进程都被**显式 kill + wait 回收子进程**，零残留。
...
1. `SessionRegistry` 加**显式** `shutdown_all()`：逐个 `Transport::shutdown()` → `wait()` 回收子进程。
...
| 路径 | 结果 |
|---|---|
| 关窗口退出 | ✅ E2E `exit_residue` 通过：忽略 SIGHUP 的探针 pid 1739 随会话被回收（原始输出为「零残留」）；app 进程确实退出 |
| panic | ✅ 临时把 `greet` 改成 panic 后实测：hook 先输出崩溃现场 → `shut_down=2 failures=[]` → 日志有两条「会话已显式回收」→ `abort()`；探针同步消失 |
| `tauri dev` 重载 | ❌ **无法做到，且原因已确认**：上游 `run_dev_watcher` 用 `child.kill()` → `SharedChild::kill()` = **SIGKILL**，进程没有任何执行代码的机会。实测 `just dev` + 改 Rust 文件：新一代 app 已启动，上一轮的探针 **880 仍存活** |
...
- `Sessions::shutdown_all()`：整份搬出清单（不握锁 wait）→ 逐个 `Transport::shutdown()`
  （载体自己保证 kill + wait 回收子进程）→ 一起注销注册；幂等；失败**只登记、不向上抛**。
- 挂点：`RunEvent::Exit`（**不用** `ExitRequested` —— 阶段 3 的"收托盘"正是在那里 `prevent_exit`）
  与 panic hook。

### 与计划的偏差（都有理由）

1. 步骤 3 的「先停读循环」**无法做到**：阻塞在 `read()` 上的线程没法定点打断（`close` fd 对阻塞读
   不可靠，`read` 又没有超时接口）。**kill 才是停它的手段** —— 所以顺序是
   「收会话 → kill 子进程 → wait」，读循环在被 kill 后自行带出 EOF 并结束。
2. 只做 `Transport::shutdown()`（= kill 那个 shell）**修不了本 plan 要修的问题**：
   忽略 SIGHUP 的进程对它不可达。会话级回收是必需项，不是附加项。
...
### 一并查出的两件事（已修，记进 STATUS 的已知问题）

- `tauri-plugin-log` 默认带的 `TargetKind::LogDir` 在**日志目录不可写时会让插件初始化失败**，
  而插件失败 = **app 无法启动**（实测：`PluginInitialization("log", "只读文件系统")` 直接 abort）。
...
- **SIGTERM / SIGINT**（`kill <pid>`、Ctrl+C）：进程直接死，钩子无法执行。今天的行为**不比改前差**
