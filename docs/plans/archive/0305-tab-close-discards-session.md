...
- **前置**：plan 0204（显式 kill + wait 回收子进程）、0205（进程外兜底）—— 关闭路径的机制已经成立
- **状态**：已完成
...
3. 关闭**最后一个**标签页是**空状态**（界面空了、进程还在），**不是退出应用** ——
   「关标签页 ≠ 关窗口 ≠ 退出应用」（托盘与退出是阶段 3 另一条线的事）。
4. 把"哪一类标签页可关闭"写成规范：
   - **三大终端**（local / ssh / serial）的标签页：**有关闭按钮**，关闭 = 立刻丢弃该 `Session`；
...
4. **前端不等回收完成**：`void close()`。"立刻" = **没有宽限期**，不是"UI 阻塞到子进程收完"；
   真正的 wait 与回收子进程在后端的 `Transport::shutdown()` 里（0204）。
5. **焦点交接**：切到某个标签页时把键盘焦点交给 xterm 的输入框（否则用户点完标签页按键没反应）。
   只碰焦点，不碰终端内容（§4.2 禁止的是绕开 xterm 改 DOM 文本）。
6. **E2E**（`src-tauri/tests/tab_close.rs`，接入 `E2E_TARGETS`）：真实 app、真实点击，判据见下。
...
```bash
just dev
# 1) 开一个终端，运行 `seq 1 200`；2) 点 + 开第二个；3) 切回第一个 —— 回滚内容必须还在
...
**这条 E2E 第一次运行就失败，抓到的是一个真实 bug**（不是测试写错）：

1. **`term.dispose()` 会抛，而它运行在 React 的 effect 清理函数里。** 触发状态：终端的 WebGL
   ...
   修法两条：`surface.dispose()` 捕获异常（只登记），`attach.ts` 的清理**先交会话、后拆面**。
   用例因此**故意**先把那个状态造出来（`LOSE_WEBGL`），失败与通过都实测过。
2. **多标签之后 `.xterm-helper-textarea` 不再唯一**：`terminal_render` / `exit_residue` 的输入
   ...
   可能落进一个隐藏的终端。
3. **探针原本是"最后挂载的那个面"**，多标签下会与"用户正在看的那个面"分叉（切回去、关闭别的
...
- **前端类型检查不在 `just ready` 里**（它只覆盖 Rust + 文档）：本步靠 `pnpm build` 手动运行，并以 E2E
  实际运行兜底。要不要把 `tsc` 纳入门禁（牵涉 CI 那条 job 装不装前端依赖）是**另一件事**，记在 `STATUS.md`。
