...
- **不**引入 async runtime 决策（这条属于阶段 2 的实现细节，但不得同时引入 `tokio`）

## 前置检查

```bash
cd src-tauri                                    # 仓库根没有 manifest（问题 #8）
...
2. 两条触发条件：
   - **容量**：累计 ≥ 64KiB → 立刻交付
   - **时间**：距本批首字节 ≥ 16ms → 交付
3. EOF / 错误 / `shutdown` 路径必须 `flush` 残批（**不得丢弃尾部残批**）。
...
整组运行完约 30 s（criterion 的统计开销占大头，不是被测代码慢）。

**重复运行一次**（同机、同二进制、隔了几分钟）：51.1 GiB/s / 14.4 ns 每批 / 9.37 GiB/s ——
三组都在 3% 以内浮动，方向上还有正有负。这正是「基线不是门禁」的实证
（`AGENTS.md` §7）：同一份代码重复运行都有这个量级，拿它当通过条件只会得到随机红。

### 手段上的一处追加：`spawn_batcher`（计划外，但必要）

计划只写了合批器本体（`push` / `poll_due` / `flush`）。落地时发现**仅有合批器本体无法与读取端对接**：

`Box<dyn Read + Send>` 是阻塞的，也没有可移植的超时接口。若读循环"读一块→喂一块"，
**时间触发永远不会单独发生** —— 它会退化成"下一块字节到来时才一并交付"。这不是
理论问题：shell 打完提示符 `> ` 之后就没有输出了，用户正**等这个提示符**，而它会
一直留在缓冲区里直到用户按键。只按容量合批的实现在交互场景下不成立。
...
由此，边界数字全部由**注入假时钟**的纯逻辑单测固定（精确到 1 ms / 1 字节）；
驱动那三条只断言"内容"与"最终一定会交付"（5 s 上限等待 50 ms 的工作），不看调度时序 ——
否则断言依赖的就是调度时序的偶然性。
...
单测全绿"。已改成 `batcher`（`docs/STATUS.md` 问题 #27）。

### 与计划的偏差

- **`bench` 成为配方**（新增 `just bench`，配方 19 → 20）：基线要反复运行、要能对比，
  就必须有一个被 `docs-check` 登记的入口，不能只存在于 plan 的代码块里。
- 同时修了 plan 0105 以来一直沿用的 `cargo …` 写法：仓库根没有 manifest（问题 #8），
