...
- **前置**：plan 0403（四类池的 CRUD —— 没有数据就验不了"数据还在"）、
...
把 [`../../portable.md`](../../portable.md) §5 的五步**做成一条可运行的配方**，并补上 §4 第 3 条
...
- 不做 `strace` 类全量写入审计（判据已简化为"移动文件夹后还能开、数据还在"）
...
| 量什么 | 结果 | 结论 |
|---|---|---|
| 便携目录**可写**时 | `config.json` 确实被读到（`lifecycle` probe 报 `close_behavior=exit`），库也落在它里面 | "数据目录从 bin 推导"已经对，缺的只是迁移那一步的验证 |
| 便携目录 `chmod 500` 时（**今天**） | app **照常启动**，日志只有一条 `config not found path=<那个目录>/config.json` | §4 第 3 条**没实现**；而且那条日志还把"无法写入"说成了"没有配置文件" —— 正是要消掉的那个静默 |
| 把 debug 二进制**复制**到临时目录再启动 | 启动成功；Victauri 的端口/令牌照常出现在 `<temp>/victauri/<pid>/` | 五步能自动化 —— 被测的是"bin 在哪"，不是"构建目录在哪" |
| `vault_unlock` 这类 `invoke_command` 走哪条路 | 走 webview 里的 JS bridge：Vite 不在时返回 `bridge not responding` | 这一段必须在 Vite 已启动的环境里运行（`just test-e2e` 本来就有这条件） |
| `app_state`（probe） | **不过 bridge**，Vite 不在也能读 | "app 挑了哪个数据目录"用 probe 观察，不用 grep 日志反推 |

## 四个决定

1. **被测对象是"复制出来的那一份 bin"**。`just test-e2e` 用的 app 运行在 `target/debug/` 里，
   无法移动它（那是构建目录）。所以这一段**自行启动 app**：把 `CARGO_BIN_EXE_akasha`
   复制进临时布局 `A/{akasha, akasha-data/}`，在 A 启动一次、移动为 B、在 B 再启动一次。
   ⚠️ 代价是**不能与别的 akasha 同时运行** —— 单实例（plan 0304）会让第二份自己退掉，
   于是"app 无法启动"看起来像可搬迁性故障。所以配方先查一遍并说清原因。
2. **便携目录不可写 → 一条 `error` + 退出码 2，不弹窗**。判"可写"用**实际写一个探针文件**
...
3. **判据分两半，写在同一条用例里**：app 侧（probe 报的路径 + `vault_unlock` 读回四类池行数）
   与库侧（移动完成后用同一口令打开，四行**内容**逐项一致）。只验 app 侧不够 ——
   库侧才是"没有静默丢内容"。
4. **不进 `just ready`**：它要 GUI + Vite，与 `just test-e2e` 同一类。形态是一条新配方
   `portable`（`just --list` 里可见），由 `just test-e2e` 的自起分支**再运行一遍** ——
   E2E 入口仍然只有一处（`AGENTS.md` §12），CI 三个平台因此也一并覆盖到。
...
3. `tests/portable.rs` 两条用例：
   * `data_survives_the_move`：A 启动（probe=exit、`vault_status` 在 A、解锁建库）→ 停 →
     用库函数灌四类池各一行 → `mv` A→B → B 启动（probe=exit、路径在 B、解锁读回 1/1/1/1）→ 停；
   * `an_unwritable_dir_refuses_to_start`：布局 `chmod 500` → 退出码 2 + stdout 里有那条 error。
     先做**正对照**（这台机器上确实无法写入）—— 确实写不进去才有意义，否则显式跳过。
4. `src-tauri/justfile`：新增 `portable` 配方（Vite + `cargo test --test portable`）；
   新增 `E2E_SELF_APP` 清单并纳入 guard（这个目标**自行启动 app**，不能由 `run_targets` 运行）；
...
just portable   # 可搬迁性：迁移五步 + 不可写拒绝（Vite 由配方自己保证）
just test-e2e   # 全部通过；自起分支的第三段调用的就是上一条
...
| # | 判据 | 机器可查处 |
|---|---|---|
| 1 | 便携目录存在时，数据**落在 bin 同目录** | `tests/portable.rs`：A 启动的 app 报的路径在 A 里 |
| 2 | 移动后 app 仍能开，且**认的是新位置** | B 启动的 app 报的路径在 B 里；`lifecycle` probe 读到跟着走的 `config.json` |
| 3 | 原有**主机/密钥/规则都在** | B 启动的 app `vault_unlock` 读回 4 类池各 1 行；库侧逐项比对内容 |
| 4 | 便携目录不可写时**启动即报错**，不静默退回 | 退出码 2 + `portable data dir not writable` |
| 5 | 这些结论不靠日志反推 | 前三条读的是 probe 与 IPC 返回值；日志只在第 4 条（那正是被验的行为） |

## 回滚

纯增量：删 `tests/portable.rs`、`portable` 配方与 `.setup()` 里那一段即可回到当前状态
...
| `src-tauri/tests/portable.rs` | 3 条用例：迁移、不可写拒绝、**没有便携目录时不许拒绝**（正对照的另一半） |
...
B 解锁读回四类池 = {"forwards":1,"hosts":1,"keys":1,"serials":1}
...
**写用例时发现的两件事**（都不是产品 bug，都与"就绪"这个概念有关）

1. **发现目录出现 ≠ app 就绪**：Victauri 的插件 setup 比 app 自己的 `.setup()` 早，
   所以刚连上时 `lifecycle` probe 还是 `{"initialized": false}`（`record()` 在 `.setup()` 里，
   而拒绝启动的检查在它前面）。第一版用例把那个中间态当成了答案，失败在
   `probe 里没有 close_behavior：{"initialized":false}`。改成**等那个字段自己出现**。
   （同一条理由还有第二层：`invoke_command` 走 webview bridge，是最后才就绪的一个，
   所以 `vault_status` 也改成"等它实际应答一次"。）
2. **库是 app 建的，不是我们建的**：第一版的 `seed()` 用 `create()` → `VaultExists`。
   顺序是有意的（第 1 步要验的正是"app 自己把库建在便携目录里"），所以 `seed()` 改用 `open()`。
3. **`chmod 500` 那个 fixture 在单测里不可行**：`no-println` 规则豁免的是 `**/tests/**`，
   **不豁免** `src/` 里 `#[cfg(test)]` 模块里的 `eprintln!`（那条"跳过并说明原因"因此失败在 lint 上）。
   改成**结构性**造法：把探针路径指到一个普通文件底下（`ENOTDIR`，root 也无法绕过）——
   于是它不依赖权限、平台或文件系统。`chmod` 那条留在 `tests/portable.rs`：
   那里要的是 **app 确实看见一个不可写目录**，而它本来就有"跳过并打印原因"的豁免。
...
- **没有在本机验 Windows / macOS**：`chmod` 那条正对照在 Windows 上会显式跳过（并打印原因）；
  另两个平台的覆盖交给 CI 矩阵 —— 但 CI 至今未运行过（无 remote），照实记录。
- **复用别人 app 的那一支不运行这一段**（单实例会让它启动的第二份自己退掉），
  `test-e2e` 会打印原因；想验证就单独运行 `portable`。
