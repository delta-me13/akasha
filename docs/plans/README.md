...
| 编号 | 标题 | 状态 | 前置 / 展开时机 | 文件 |
|---|---|---|---|---|
| 1101 | 串口 `Session` 接入 app（命令 + 注册表 + 标签页 + 关闭与回收） | 已完成（2026-09-15：设备 → 界面 → 设备两个方向都在真实 app 上走通；关标签页后注册表回到打开前的读数；E2E `serial_session` 1.00 s；`just ready` 6/6） | 前置：plan 0801 / 0802（已完成） | [1101](./archive/1101-serial-session-ipc.md) |
| 1102 | 端口枚举与参数接进界面 | 已完成（2026-09-15：界面上一屏看齐三条并列输入 —— 枚举到的端口 / 手输路径 / 池行取值；点一条枚举结果只把它填进路径栏；手输 PTY 从端打开并双向传字节；数据位填 9 时那个面的报错行里是 `data_bits = 9` 且关闭之后注册表回到打开前，填 `abc` 则由面板自己拦下、不开面；`just ready` 6/6） | 前置：plan 1101（已完成） | [1102](./archive/1102-serial-ports-ui.md) |
| 1103 | 设备消失时串口会话以可读原因结束 | 已完成（2026-09-15：关闭 PTY 主端（等价于拔掉设备）→ 读端报 `BrokenPipe` → 会话自己结束、标签页随之关闭；界面上那句话是"串口设备已断开：<设备路径>（Broken pipe）"，注册表回到打开前的读数；E2E `serial_device_gone` 0.36 s；`just ready` 6/6） | 前置：plan 1101 / 1102（已完成） | [1103](./archive/1103-serial-device-gone.md) |
