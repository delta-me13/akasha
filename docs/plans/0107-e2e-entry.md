...
（问题 #158 / #159 / #160 记了这三件事与处置）。**Windows 那格更早就停住了**：13 分 38 秒的构建之后，配方报 `❌ app 没起来`，而那份 app 日志
**一个字节都没有**（问题 #162）—— 三个平台里只有它连第一个目标都没执行到。**还没验证的**：
修正之后的三格结论 —— macOS 的 `bw_import` 是否随串口跳过一起转绿；Windows 上 app 为何起不来
（要抓的证据见问题 #162，那三条判活 / 回收路径与三条串口目标在 ConPTY 上的跳过都还没有读数）。
