...
| `just portable` | 可搬迁性：把 bin 所在文件夹整个移动之后数据还在吗（`docs/portable.md` §5 的五步，外加 §4 第 3 条"便携目录不可写就拒绝启动"）。自行启动 app：把二进制复制进临时布局，在 A 启动一次、移动为 B、再启动一次，两次都用 app 自己的命令读回四类池，并用库函数逐项比对内容。⚠️ 不能与别的 akasha 同时运行：单实例（plan 0304）会让它启动的第二份自己退掉，所以先查一遍并说清该关闭什么；Vite 复用或自起（与 `just test-e2e` 同一套做法）。`just test-e2e` 的自起分支在第三段调用它，复用别人的 app 时那一段显式跳过并输出原因 | 转发 |
