...
- ✅ **这条契约已落地**（plan 0504）：`src-tauri/src/ssh.rs` 在启动时建**一个**专用 runtime
  （4 个 worker —— 提问会占住一个，见 D16），把 `Handle` 注入 `akasha-ssh`；起不来只降级
  （一条 `error` 日志 + SSH 命令回一句明确的话），**不挡启动**。
...
### D16 —— 提问往返：**事件 + 三条回答命令**，超时 120 s；库锁着就不连（plan 0504 定案）

- **决定**：`russh` 那条同步回调里要问用户的两件事（凭据 D8、没见过的主机密钥 D11）走**同一条**
  往返：后端 `emit` 一个事件（`ssh_prompt`，判别式是主机密钥 / 凭据），前端答三条命令之一
  （`ssh_prompt_credential` / `ssh_prompt_host_key` / `ssh_prompt_cancel`）；后端被撤下时再
  `emit` 一次 `ssh_prompt_dismissed`。**超时 120 s，超时即拒绝**（不是接受，也不留一条悬着的连接）。
  **库锁着就不连**（信任记录与密钥池都在库里），"读不到信任记录"一律拒绝而不是当作未知。
- **理由**：
  - **超时是安全阀也是上界**：提问期间连接线程停在一次同步回调里（那条回调占着 SSH runtime 的
    一个 worker），没有人回答就永远不返回。120 s 既够用户去看一眼 2FA 验证码，也把"一个 worker
    最多被占多久"钉住。
  - **答案只走命令、不进事件**：事件是**朝向**前端的（谁都能看），而口令只许从前端往回走 ——
    于是它经的入口只有一个（`PassphraseInput`，**没有 `Debug`/`Clone`**），进去立刻变受保护页。
  - **"没答" ≠ "拒绝"**：主机密钥那一问只认"接受 / 拒绝"；超时 / 取消 / 答错类型一律是
    **拒绝连接**（`HostKeyUnknown`），绝不能读成"用户接受了"。
  - **前端走了不能把连接线程永远留着**：这是超时与 `ssh_prompt_dismissed` 一起解决的事；
    用户也可以从提示面板显式取消（= 让连接以"用户取消"结束）。
- **否决的替代路**：① 让 `akasha-ssh` 自己 `emit` —— 它不依赖 Tauri（`crates/*` 不得
  `use tauri::*`），而且"问谁"是 app 的事；② 把提问做成一次普通命令的返回值（前端轮询）——
  连接线程等答案的地方在同步回调里，轮询要么丢掉答案、要么把回调变成一次忙等。
- ⚠️ **代价照实记**：提问是**同步阻塞**的（trait 就是同步的），所以它会占住 SSH runtime 的
  一个 worker。缓解 = D2 那个 runtime 的 worker 数 + 上面这个超时，两者缺一都会让一条没人理的
  连接把 worker 攥住。
...
"报错"与"警告"的分界线是**后果**，不是"认不认识"：

| 档 | 判据 | 行为 |
|---|---|---|
| **导入** | 六条 | 映射进 ssh 配置池 |
| **警告** | 忽略它**不改"连到哪台机器"**、也**不改"信任哪把主机密钥"**，而且"没生效"会逐条出现在报告里 | 逐条列出并继续 |
| **报错** | ① `Match` / `Include`（条件与内容都不在我们读到的文本里）② 改目的地或解析路径（`ProxyCommand` / `Canonicalize*` / `BindAddress` / `AddressFamily` / `Tunnel`…）③ 改信任来源（`HostKeyAlias` / `RevokedHostKeys`）④ `IgnoreUnknown` ⑤ **表里没有的** | 整份不导入，**一次列全**（带行号） |

⚠️ 最后一格是方向：**没见过的一律报错**。于是"漏判一条会改目的地的关键字"的后果是它落进报错
那一档，而不是静默放行。规则与名单只有一处（`akasha-store::sshconfig` 的 `classify`）。

三条同批定下的细节：

- **`ProxyJump` 只收裸名字**（`host`，逗号分隔算一串）与 `none`；`user@host` / `host:port` /
  ssh URI **报错**并给出改法。理由：池里一跳就是**一行**，而 `user@` / `:port` 是"对某一跳的
  局部改写"，库里没有对应的表示 —— 硬塞会变成一条用户没写过的行。
- **`IdentityFile` 只兑现一半**：私钥**不导入**（导入配置与导入机密是两件事，后者要按
  ADR-0002 D13 重验），条目按 `publickey` 落库、`key_id` 留空（= 走 ssh-agent），报告里逐条说明。
- **求值语义以 `ssh -G` 的实测为准**（系统 OpenSSH 10.5）：`Host *` 写在前面时它的取值**压住**
  后面的具体条目（"首次取到的值生效"）；文件开头到第一个 `Host` / `Match` 之间是**全局段**；
  **关键字**不分大小写而 **`Host` 模式**分大小写；没写 `HostName` 时目标名被小写化；
  **通配块不产生条目**，它的取值靠匹配并入具体条目。
...
| 代价 | 对策 / 现状 |
|---|---|
| 进程里多一个 tokio runtime（与 tauri 自己那个并存） | 只在 `akasha` 建一处、worker 数可配；退出路径先 `shutdown_all` 再 drop runtime（顺序不能反）。✅ plan 0504 落地时的形态：runtime 由 `State` 持有，`RunEvent::Exit` 里先收会话，runtime 在 tauri 拆 `App` 时才 drop —— 顺序由生命周期保证，不需要手写两次 |
...
| 2026-09-13 | **D14 展开**（plan 0506）：补上三档边界（导入 / 警告 / 报错）与各自的**判据**、`ProxyJump` 只收裸名字、`IdentityFile` 只兑现一半、求值语义的四条实测依据（`Host *` 在前者胜、全局段、关键字与模式的大小写、通配块不产生条目）；**`Match` / `Include` / 未知关键字仍然报错**，决定本身一字未改 | D14 只写了"看不懂就报错"，而真实配置里满是与连接无关的指令（`ServerAliveInterval`、`IdentitiesOnly`、`LocalForward`…）—— 照字面执行会让这个功能在**任何一份真实配置上都失败**，于是"受限子集"退化成"谁都用不了"。分界线因此从"认不认识"改成"**忽略它会不会连到别的机器、或改我们信任哪把主机密钥**"，并把"表里没有的一律报错"留成方向 |
