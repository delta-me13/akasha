...
- **状态**：**已定案**（Frozen，2026-09-11）
  —— 阻塞项（决策二）已由 cyrene 裁定，见 §0.3。§6 的未决问题**不阻塞定案** ——
  它们是实现到那一步时才需要回答的问题。
- ⚠️ **决策一（根 workspace）已被 [ADR-0004](./0004-rust-workspace-under-src-tauri.md) 取代**
  （2026-09-11）：workspace root 改为 `src-tauri/`，成员在其 `crates/` 下，仓库根不放 Rust 成员。
  本文其余部分（分层、命名、依赖方向、决策二）**继续有效**。
  下文出现的 `crates/…` 路径按**当时**的布局书写，请对照 ADR-0004 读作 `src-tauri/crates/…`。
- **日期**：2026-09-11
- **决策者**：cyrene
...
## 0. 补记（2026-09-11）：范围扩大后的影响 + 决策二裁定

> 本文写于"产品 = 一个终端"的假设下。此后产品范围扩大为**多后端（local/ssh/serial）
> + SFTP + 凭据库 + 端口转发 + 常驻托盘**（见 [`../scope.md`](../scope.md)）。
> 这带来两处影响，和一条裁定。
...
ssh / serial / sftp / store / 托盘与隧道都会各自成 crate。
**§3.1 的分层图是起点而非终点**；但两条依赖方向规则对新 crate 一律适用：

- 单向：`src-tauri` → `crates/*`，反向视为违规
- `crates/*` 不得 `use tauri::*`

### 0.3 决策二的裁定（cyrene，2026-09-11）
