...
- **状态**：**已定案**（Frozen，2026-09-15；落地它的 plan 0701–0704 全部完成并归档）
- **日期**：2026-09-15
- **决策者**：cyrene
- **影响范围**：新增依赖 `russh-sftp`；`akasha-ssh` 多一个 SFTP 会话类型与其失败档；
  `SessionKind::Sftp` 拥有的资源形状；传输引擎的端点职责边界（plan 0702 / 0703 / 0704 的地基）；
  SFTP 的 IPC 契约（命令、`side` 的表示、只读探针）
- **关联**（**全部已归档**）：[plan 0701](../plans/archive/0701-sftp-dual-pane.md)（本文的落地起点）、
  [0702](../plans/archive/0702-transfer-atomic-rename.md) /
  [0703](../plans/archive/0703-host-to-host-topology.md) /
  [0704](../plans/archive/0704-pipelining.md)
- **取代**：无
...
本文只定义形状与约束，不含实现步骤 —— 步骤在 plan 0701–0704（**均已归档**）。
按 [`docs/adr/README.md`](./README.md) 的三态规则，本文已**定案**：§1–§6 **不可再改**，
实现期的改动全部记在 §6（那一段是它从「实现中」走过来的痕迹）；
要推翻这里的任何一条，只能由新的 ADR 取代它。
...
| **A（回退）** | A 上的 `SftpClient` | B 上的 `SftpClient`（本机分别连 A 与 B） | 本机必须能直达 B |

于是"优先 B、失败回退 A"是**选哪一对端点**，引擎一行都不用改；而 B 档消费的正是
ADR-0003 D9 的那条流，不另写一套通道逻辑（`AGENTS.md` 的"只实现一次"）。

plan 0703 落地时把上一版留给它的三件事定了下来：

- **档位在"连接那一刻"定，定的是目标那一栏的端点**：`sftp_connect` 在连 B 之前先看另一栏 ——
  它已经连上一台**不同的**主机 A 的话，先建"本机 → A → B"这条链（B 档）；建不起来再改走
  本机直连（A 档）。于是"这次传输走的是哪一档"是**目标端点怎么来的**的后果，
  `sftp_transfer` 与传输引擎都不做档位判断。
- **那条链是一条独立的连接**（本机 → A → B），**不复用另一栏的连接对象**：另一栏换主机 /
  断开都不会带走这一栏，D3 的"不复用同一条连接"因此仍然成立。A 那一行自己的跳板链与
  B 那一行自己的跳板链都保留（拼法只是两段材料首尾相接），收尾顺序仍由 `akasha-ssh`
  那一处定死。
- **哪些失败算"B 不可用"**：那条链建链尝试的**任何**失败都算 —— A 拒绝转发、A 连不上 B、
  到 B 的认证失败、超时。**不分类**：对"要不要回退"这个问题它们没有区别。回退是**正常
  路径**（对端不认这条转发是常见配置），但必须留下证据：`SftpSideInfo.through`（经哪台
  直通）与 `through_failure`（回退原因），以及传输记录里的 `via`。
- **方向**：通道开在"到**源那一栏所连主机**的连接"上，于是 B 档的方向是**源 → 目标**；
  目标那一栏是可直达的那台时，这次传输的形态就是本机内存中转。方向由用户选的来源决定，
  不由"哪台更近"猜。

⚠️ **"优先 B"的收益是可直达性，不是字节数**：两档下文件都要过一次本机 —— 读源一遍、
写目标一遍，`scope.md` §4.1 的"本机带宽减半"按字面不成立（那要靠在 A 上执行
`rsync`，即 §10 的 `no`）。B 档真正解除的是"本机必须够得着 B"这条限制：本机只需要能连上 A。
plan 0703 的实施记录按这个口径写（两档的本机侧字节数相同）。
...
plan 0704 把这些落成了四样东西：

- **上限是引擎的一个类型**（`akasha_ssh::InFlight`）：`limit` 是参数，`live` / `peak` 是
  "上限真的在起作用"的读数（探针 `sftp` 的三个数、面板表头那一句都由它们来）。
  有界入口是它唯一的方法；`transfer()` 本身仍无上限 —— 那是一次搬运，不是一种策略。
- **空位属于会话，不属于某一次调用**：一个 SFTP 会话持一个 `InFlight`，于是上限约束的是
  这个会话的全部传输，不区分它们来自几条命令（app 的形状是"一个文件一条命令"）。
- **等空位是可以取消的**：关会话与用户取消推的是同一个信号（D4），排队中的传输因此在
  **碰目标之前**退出 —— 否则它会先建一个临时文件再删掉，而"关会话"要等它走完这两步。
- **临时名的占用改成原子的**（原来的那条判据因此不只是一个判据，见 §4）：本机用
  `create_new`（`O_EXCL`），远端用 `CREATE|EXCLUDE`，都是**一次**调用 ——
  "先查存在、再创建"那道缝没有了。
...
- 只读探针 `sftp` 报两侧的状态、当前路径与**到达方式**（`through` / `throughFailure`），
  以及每条传输的 `via` —— 判据的读数口（`AGENTS.md` §7 的探针纪律）。
  这三个字段是 plan 0703 补上的（见 §6）：B 档与回退都必须能从探针里读出来，
  否则"这次走的是哪一档"只能靠日志反推。
  每个会话还报一对总不是空的东西：`inFlight.{limit, live, peak}`（plan 0704 补上）——
  上限与"此刻/最多几个文件在搬"。⚠️ 排队中的传输在传输记录里与"正在搬"长得一样
  （状态只有"还没结束"这一档），分辨它们靠 `live` 比"还没结束的条数"少。
...
- **吞吐有了数字，但分档曲线有一段没解释清楚**：12 个 1 KiB 文件、一条每个方向延后 10 ms
  的链路，上限 1 = 1.17 s、2 = 1.16 s、4 = 0.56 s、8 = 0.38 s、16 = 0.21 s（plan 0704 的
  实施记录）。**上限 2 与串行一样慢**：那一刻服务端记到的两次 `open` 只差 0.08 ms（请求
  确实是并发发出去的），两条传输也几乎同时结束 —— 于是每一方都慢了一倍。机制尚未定位
  （候选：测试服务端串行处理请求，与同一条连接上两个文件各自的写请求之间的相互等待）。
  默认值因此**不按"饱和点"取**，按占用与吞吐的折中取（`akasha_core::Transfer::default`）。
- **临时名的占用现在由对端保证**（这一条由 plan 0704 落地）：本机 `create_new`、远端
  `CREATE|EXCLUDE`，都是一次调用。⚠️ 远端那一侧**分不清**"名字被占"与别的通用失败
  （SFTP v3 的状态码里没有"已存在"这一档），所以只有通用的 `Failure` 换下一个候选，
  别的错误当场报出去 —— 一条只认 `Failure` 的规则是这一档协议能力的下界。
- **两档都没有与真实 `sshd` 对照过**：B 档（`direct_tcpip` + 隧道里的 SFTP）与 A 档都只在
  测试服务端上验过。真实的 `AllowTcpForwarding no`、`PermitOpen` 白名单、以及转发的
  带宽/缓冲特性都还没有对照 —— 它们改变的是"失败长什么样"，而回退的触发点
  （建链失败即回退）不依赖具体是哪一种。
- **`SshConnection::over` 不持有承载它的那条连接**：它只造下一跳，链的存活归调用方
  （`chain` / `connect_via` 会把整条链放进 `under`）。app 那条路走的是后者，所以不受影响；
  这条记在这里是因为"用 `over` 单独建一跳"看起来像能独立存在，而它不是。
...
- **2026-09-15**（plan 0703 落地时）：D5 把留给它的三件事定下来 —— 档位在**连接那一刻**定
  （目标是哪一档由目标那一栏的端点怎么来体现）、隧道是一条**独立**的链（不复用另一栏的
  连接对象）、"B 不可用"= 那条链建链尝试的任何失败；并记下"优先 B"的收益是**可直达性**
  而不是字节数（`scope.md` §4.1 的"带宽减半"按字面不成立，同一处已按这个口径更正）。
  D7 的探针那条补上 `through` / `throughFailure` / `via`；§4 换掉"回退判据未定"，
  改为"两档都还没与真实 `sshd` 对照过"与"`over` 不持有承载它的连接"。
  落地时**引擎一行都没改**（D5 的预判成立）：`direct_tcpip` 原语、`sftp()`、
  `connect_via_until` 全是既有形状。
  ⚠️ 由此 ADR-0003 D9 里那句"SFTP 的 B 档仍未接（plan 0703）"与 §12 的同一条**成了过去时**。
  那份 ADR 已定案（不可改），所以它的正文一字未动 —— 现状以 `docs/STATUS.md` 为准，
  这一行就是那条漂移的留痕。
- **2026-09-15**（plan 0704 落地时）：D6 把一个上限落成 `akasha_ssh::InFlight`（参数 +
  `live` / `peak` 两个读数）、空位归**会话**、等空位**可取消**，并把临时名的占用从
  "先查存在、再创建"改成**一次**原子占用（本机 `create_new`、远端 `CREATE|EXCLUDE`）——
  §4 里"占用探测不是原子的"那一条因此不再成立，换成远端那一侧的**分不清**（v3 没有"已存在"
  这一档，所以只有通用的 `Failure` 换下一个候选）。§4 的"吞吐没有数字"换成实测的
  分档数字，并记下**上限 2 与串行一样慢**那段还没定位的曲线（默认值因此不按饱和点取）。
  D7 的探针那条补上 `inFlight.{limit, live, peak}`。
- **2026-09-15**（阶段 7 收尾时）：状态由「实现中」转为**已定案** —— 落地本文的 0701–0704
  全部完成并归档。同时把 §4 里两条已被实现推翻的表述改到最终形状：并发下临时名的
  唯一性由**对端保证**（本机 `create_new` / 远端 `CREATE|EXCLUDE`），以及默认值**不按
  "饱和点"取**（分档实测里 16 仍明显更快，8 是占用与吞吐的折中）。此后本文不可再改。
