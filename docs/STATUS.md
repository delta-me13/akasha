...
它的第一项（ADR-0002 定案）**草案已写、等裁定**（见「进行中 / 下一步」）。
...
- [ ] **阶段 4 的第一项**：**ADR-0002**（存储与机密来源；动存储代码之前）——
  [plan 0400](./plans/0400-adr-0002-secret-storage.md) **进行中**：
  [草案](./adr/0002-secret-storage.md) 已写（12 条决定 + 6 条被否路线 + 迁移路径 + 7 项待实测），
  状态**提议中**，等 cyrene 裁定。裁定后 plan 实施记录里列的 5 项收尾（状态改「已接受」、
  `scope.md` §6/§7 补指针、plan 0403 那条据 D11 改掉、归档、勾 ROADMAP）。
  ⚠️ 本节原先挂的"`config.json` 应并进 DB"这条**迁移债已据 D11 取消**：关窗行为必须在**解锁之前**
  可用，所以明文配置留在库外 —— ADR 接受后同步 plan 0403 的前置检查。
- [ ] **ADR-0002 §7 的 7 项实测**（现在全是"预期"）：`PRAGMA cipher_settings` 的实际输出、
  错误口令与**空口令**的报错形态（空 key 在 SQLCipher 里 = 关闭加密）、导出 round-trip、
  `rekey` 后盐是否变化 —— 排在 plan 0401 的第一件事
...
### 本轮完成（plan 0400：ADR-0002 草案）

阶段 4 的第一项是"动存储代码之前先把数据文件格式定案"。草案已写：
[`docs/adr/0002-secret-storage.md`](./adr/0002-secret-storage.md)（**提议中**，等裁定）。
...
- [ ] **未做**：ADR 的接受动作（改状态、`scope.md` 补指针、归档 plan、勾 ROADMAP）——
      接受是 cyrene 的裁定，见「进行中 / 下一步」
