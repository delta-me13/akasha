...
- [ ] **扩范围后的文档收尾**（能力已记录在 `docs/scope.md`，下面这些还没做）：
  ...
  - [x] 连接模型定案：**不复用** —— 每个终端会话/隧道各建一条 SSH 连接
        （副作用：同主机开多标签会重复认证 → 需内存凭据缓存 / SSH agent / known_hosts 缓存）
  - [x] **连接生命周期定案：连接 = 拥有它的 tab 的生命周期。**
        关闭 tab → **立刻**关闭其连接（无宽限期）；窗口收托盘（隐藏）→ 不受影响。
        推论：托盘**必须隐藏窗口而非销毁**（销毁会销毁 tab → 立刻断全部隧道）；
        关闭 tab 必须同时中止该 tab 的重连循环与进行中的传输
  - [x] 重连策略定案：**有限次（带退避）后标记失败**，不做无限重连
        → 必须有状态机 + 事件 + 托盘可见 + 手动重试，否则退化成**静默失败**
  - [ ] **新增能力尚未细化的部分**：
        - 托盘的 Linux 依赖 `libayatana-appindicator3` 已在 CI apt 列表里，需实测
        - 单实例处理（第二个实例应唤起已有窗口，而不是各跑一套隧道）
        - 重试次数与退避参数的具体默认值（要可配置，不硬编码）
        - 关闭 tab 时未完成 SFTP 传输的部分文件如何处置（删除 / `.part`）
