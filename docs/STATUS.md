...
| ↑ **同一份代码的下一次运行**（`36251661992` @ `08fffb2`，只改了文档） | ⚠️ 有两处红：Windows 的 `ssh_config_import`（"选择器里有导入面板"超时，底层的 eval 9 s 未返回）与 Linux 的 `ssh_session`（服务端回声 30 s 没出现在终端上）—— 两处都停在 `tests/support/mod.rs:244` 那个等待辅助上。**重跑这两个 job 即绿**（六格全绿），因此记为**偶发**，与本次改动无关（那两条路径不碰隧道、也不碰 `vault_unlock`）|
