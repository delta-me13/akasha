...
三个反复出现的病根，都是同一个：**把"给人读的说明"写进了日志**。

1. **括号里解释**（`（这条会话少一道兜底）`）—— 注释的活。
...
别用 `info` 报失败（会被淹没），也别用 `error` 报预期内的降级（会让红线变噪音）。

## 4. 为什么用英文

- 日志是机器接口：`target` 与依赖库（tauri / wry / rmcp）本来就是 ASCII，
  grep、管道、告警规则都不必跟编码打交道。
- 更实际的一条：**英文这个约束本身会把说明文挡在门外**。中文长句读起来像文档，
  于是就会越写越长 —— 这不是巧合，是上面的病根的成因。
...
- **字段值可能非 ASCII** —— 那是上游给的**数据**，不是我们的话术（消息仍必须 ASCII）：
  ...
  - `signal`：portable-pty 0.9 的名字取自 `libc::strsignal`，**随 locale 变**
    ...
    从它的公开 API 拿不回 `SIGKILL`。修法与代价见 `STATUS.md` 坑 #59（要动依赖）。
- 日志**格式**（`[日期][时间][target][级别]`、**无毫秒**）是 `tauri-plugin-log` 默认 formatter 的行为，
  本仓库没有自定义 —— 跨进程（app + 看门狗）的先后顺序别靠时间戳判断。
