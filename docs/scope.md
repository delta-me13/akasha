...
图例：`v1` 首版 · `later` 明确要做但不阻塞首版 · `no` 非目标（**写下非目标与写下目标同样重要**）
...
| # | 约束 | 否决了什么 |
|---|---|---|
| P1 | **不依赖操作系统组件** | `keyring` 类方案（GNOME Keyring / Keychain / Credential Manager）、任何"让 OS 帮我保管密钥"的设计 |
...
### 风险 3 — 便携 vs. Bitwarden 接入：**唯一未决项**

P1（不依赖系统组件）与"用 `bw` CLI"直接冲突。已核实的选项：
...
**倾向**：`bw` CLI（唯一可靠支持 SSH key 条目、且官方维护的路径），代价是随包分发
一个 ~100 MB 的二进制，且须把 `BITWARDENCLI_APPDATA_DIR` 指进便携目录。
**这条需要 cyrene 定案后才能写 ADR-0006。**
...
| 非目标 | 理由 |
|---|---|
| **自实现 Bitwarden vault 密码学** | §6。错了即灾难；由 Bitwarden 负责 |
| **host↔host 真不中转**（在 A 上跑 `rsync`） | §4.1。需 A↔B 互信，进度不可控 |
| **依赖 OS 凭据库** | P1 |
| **v1 写 Android 代码** | §2 |
| **Python/Node 运行时作为应用依赖** | P1（`bw` 是自包含二进制，属例外，待定案） |
| 插件系统 | 每次都会被提，v1 不做 |
| 移动端（除 Android 的 ssh/sftp 外） | v1 不做 |
