...
**一条转发规则拥有自己的连接，不挂在任何终端会话上。** 否则：

- 关掉终端标签 → 隧道断掉（用户预期它继续）
- 关掉窗口 → 隧道断掉（**这正是 §5 托盘存在的理由**）

因此数据模型上要有独立的**"隧道"实体**，而不是 `Session` 的一个属性。
...
- 隐藏窗口 vs 销毁窗口：**隐藏更简单**，且 webview 状态（xterm 回滚缓冲）得以保留。
  销毁会连带丢掉终端状态，需要重建。v1 用隐藏。
