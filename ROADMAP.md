...
- [ ] **隐藏而非销毁**窗口（点叉后进程仍在，会话与终端原样存活）
      验收：隐藏再显示后终端回滚缓冲仍在，且子进程仍在（**这是预期**，不是泄漏）
      → [plan 0302](./docs/plans/0302-hide-not-destroy.md)（**与 0303 一起落地**）
- [ ] 关闭行为可配置（收托盘 / 直接退出）
      验收：切成"直接退出"后点叉即退出且零残留；切回收托盘后点叉不退出
      → [plan 0303](./docs/plans/0303-close-behavior-config.md)（**提供"无托盘也要能退出"的退路**）
